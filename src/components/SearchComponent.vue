          ...
          <v-row>
            <v-col xs="12" md="6" v-for="(v, i) in item" :key="i">
              <v-card min-width="300" flat>
                <v-row v-if="i=='분실자명' || i=='습득물명'">
                  <v-col align="center">
                    <v-card width="95%" flat>
                      <v-text-field :label=i v-model="select[i]"></v-text-field>
                    </v-card>
                  </v-col>
                </v-row>
                <v-row v-else>
                ...
                <v-row v-if="i=='분류명'">
                  <v-col>
                    <v-select v-if="select['분류명']" :items="specific_type[select['분류명']]" :label="'세부분류'" class="pa-4" v-model="select['세부분류']"></v-select>
                  </v-col>
                </v-row>
                <v-row v-if="i=='보관지역'">
                  <v-col>
...
export default {
  data: () => ({
    select: {},
    item: {'분류명': ['가방', '귀금속', '도서용품', '서류', '산업용품', '쇼핑백', '스포츠용품', '악기', '유가증권', '의류', '자동차', '전자기기', '지갑', '증명서', '컴퓨터', '카드', '현금', '휴대폰', '기타물품'],
          '보관지역':["서울특별시", "경기북부", "경기남부", "인천광역시", "부산광역시", "대전광역시", "대구광역시", "울산광역시", "광주광역시", "제주특별자치도", "세종특별자치시", "강원도", "충청북도", "충청남도", "경상북도", "경상남도", "전라북도", "전라남도"] ,
          '분실자명':[],
          '습득물명':[]
          },
    specific_type: {
